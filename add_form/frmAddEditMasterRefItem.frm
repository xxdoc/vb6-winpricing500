...
Private Sub InitFormLayout()
   ...
   Call InitNormalLabel(lblMasterCusGroup, MapText("กลุ่มลูกค้า"))
   Call InitNormalLabel(lblMasterCusType, MapText("ประเภทลูกค้า"))
...
Dim I As Long
   ...
   For Each CheckBd In TempCollection
      ...
      If CheckBd.MASTER_CUSGROUP_ID = cboMasterCusGroup.ItemData(Minus2Zero(cboMasterCusGroup.ListIndex)) And CheckBd.MASTER_CUSTYPE_ID = cboMasterCusType.ItemData(Minus2Zero(cboMasterCusType.ListIndex)) And ID <> I Then
         glbErrorLog.LocalErrorMsg = MapText("มีข้อมูลกลุ่ม : ") & cboMasterCusGroup.Text & " และประเภท " & cboMasterCusType.Text & " " & MapText("อยู่ในระบบแล้ว")
         glbErrorLog.ShowUserError
         Exit Function
      End If
   Next CheckBd
